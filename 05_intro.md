...
- [useState](https://react.dev/reference/react/useState):
  ...
  - **Behavior:** it returns two values: the current state value and the setter (a function that allows you to update the state value). When the `state` variable is updated using the setter function provided by `useState`, it `triggers a re-render` of the component.
