...
A component changes whenever its state changes or the props change.
...
// Custom component using arrow function syntax exported with named export:
export const CustomComponent = (prop1, prop2) => {
  return `Hello from ${prop1}, I got this data from ${prop2}`;
};
```

`CustomComponent` can then be imported as follows:
...
// Parent component with inline style exported with default export:
...
- While `ParentComponent` is a React component, note that `CustomComponent` isn't, as it is just a regular JavaScript function returning a string. React expects components to be rendered using JSX syntax.
  
- Each Module can have only one `export default` keyword, but many `named exports`. A default export allows you to import a component without using curly braces {}. When importing named exports, you must use curly braces.

## Custom Components in JSX

React-native supports JSX (JavaScript XML), a syntax extension for JavaScript that allows you to write declarative UI components using a syntax that closely resembles HTML or XML.
...
// Inline style (industry practice uses styled components instead):
...
1. [useMemo](https://react.dev/reference/react/useMemo): Memoizes (caches) the output value of the provided function, and only updates if one of the dependencies listed in the dependency array changes.
